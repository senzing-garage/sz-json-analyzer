...
#        print(json.dumps(features, indent=4))
        ...
        for feature_key in features:
            ...
            if label and feature in self.label_to_attribute and self.label_to_attribute[feature] not in populated_attr_list:
            ...
            if feature == 'NAME' and 'NAME_FULL' in populated_attr_list and any(x in populated_attr_list for x in ['NAME_ORG', 'NAME_LAST', 'NAME_FIRST']):
                message_list.append(['INFO', f"Only NAME_FULL should be mapped"])
            if feature == 'ADDRESS' and 'ADDR_FULL' in populated_attr_list and any(x in populated_attr_list for x in ['ADDR_LINE1', 'ADDR_CITY', 'ADDR_STATE', 'ADDR_POSTAL_CODE']):
                message_list.append(['INFO', f"Only ADDR_FULL should be mapped"])
            if feature == 'ADDRESS' and 'ADDR_FULL' not in populated_attr_list and 'ADDR_LINE1' not in populated_attr_list:
    ...
    def get_report(self):
        ...
        for feature in sorted(self.feature_stats.keys(), key=lambda k: self.feature_stats[k]['order']):
            ...
            if self.feature_lookup.get(feature):
                ...
                if self.feature_lookup[feature]['FTYPE_FREQ'] in ('A1', 'F1') and row[5] <= self.low_f1_unique_percent:
                    ...
                    self.update_message_stats('WARNING', f"{feature} < {self.low_f1_unique_percent}% unique")
                elif self.feature_lookup[feature]['FTYPE_FREQ'] == 'FM' and self.feature_lookup[feature]['FTYPE_FREQ'] == 'Yes' and row[5] <= self.low_fme_unique_percent:
                    self.update_message_stats('WARNING', f"{feature} < {self.low_f1_unique_percent}% unique")

            i = 5
            for value in sorted(self.feature_stats[feature]['values'], key=lambda x: self.feature_stats[feature]['values'][x], reverse=True):
            ...
            if len(self.feature_stats[feature]['attributes']) > 1 or list(self.feature_stats[feature]['attributes'].keys())[0] != feature:
                for attribute in sorted(self.feature_stats[feature]['attributes'].keys(), key=lambda k: self.feature_stats[feature]['attributes'][k]['order']):
                    row = ['' for x in range(len(table_headers))]
                    row[0] = 'MAPPED'
                    row[1] = '  ' + attribute
                    row[2] = self.feature_stats[feature]['attributes'][attribute]['count']
                    row[3] = round(self.feature_stats[feature]['attributes'][attribute]['count'] / self.record_count * 100.00,1)
                    row[4] = len(self.feature_stats[feature]['attributes'][attribute]['values'])
                    row[5] = round(row[4] / row[2] * 100.00,1)
                    i = 5
                    for value in sorted(self.feature_stats[feature]['attributes'][attribute]['values'], key=lambda x: self.feature_stats[feature]['attributes'][attribute]['values'][x], reverse=True):
                        display_value = value[0:97] + '...' if len(value) > 100 else value
                        i += 1
                        row[i] = f"{display_value} ({self.feature_stats[feature]['attributes'][attribute]['values'][value]})"
                        if i == len(row)-1:
                            break
                    table_rows.append(row)
        ...
        for attribute in sorted(self.unmapped_stats.keys()):
            ...
            row[5] = round(row[4] / row[2] * 100.00,1)
            i = 5
            for value in sorted(self.unmapped_stats[attribute]['values'], key=lambda x: self.unmapped_stats[attribute]['values'][x], reverse=True):
                display_value = value[0:97] + '...' if len(value) > 100 else value
                i += 1
        ...
        for message in self.message_stats.get(old_category,{}).keys():
            if self.message_stats[old_category][message]['count'] / self.record_count >= .25:
...
if __name__ == "__main__":
    ...
    for input_row in reader:
        ...
        if input_row_count % 10000 == 0:
            eps = int(float(input_row_count) / (float(time.time() - proc_start_time if time.time() - proc_start_time != 0 else 0)))
